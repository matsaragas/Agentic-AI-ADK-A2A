...
class CoordinatorAgent:
    ...
    This is the client agent responsible for sending tasks to agents
    """

    def __init__(
            self, task_callback: TaskUpdateCallback | None = None,
    ):
    ...
    async def _async_init_components(self, remote_agent_addresses: List[str]) -> None:
        ...
        """Asynchronous part of initiation"""
    ...
    async def create(
        ...
        await instance._async_init_components(remote_agent_adresses)
        return instance
    ...
    def create_agent(self) -> Agent:
        ...
        return Agent(
            ...
            description=(
                'This coordinator agent orchestrates the product information agents'
            ),
            tools=[
                self.send_message,
            ],
    ...
    async def send_message(
        ...
        Args:
            ...
            task: The comprehensive conversation context summary and goal to be achieved
            tool_context: The tool context this method runs it
...
def _get_initialized_coordinator_agent_sync() -> Agent:
    ...
    try:
        ...
        if 'asyncio.run cannot be called from running event loop' in str(e):
            print(
                f'Warning: Could not initialize Coordinator Agent with asycio.run(): {e}. '
                'This can happen if an event loop is already running (e.g. in Jypyter)'
                'Consider initializing Coordinator Agent within an async function in your ap'
            )
        raise
